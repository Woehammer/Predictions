...
const [members, setMembers] = useState([]); const [leagueName, setLeagueName] = useState(''); const [messages, setMessages] = useState([]); const [newMessage, setNewMessage] = useState('');

useEffect(() => { if (leagueId) { fetchLeaderboard(); fetchMessages(); } }, [leagueId]);

const fetchLeaderboard = async () => { const { data, error } = await supabase .from('league_members') .select(user_id, users(username), user_points(total_points), leagues(name)) .eq('league_id', leagueId) .order('user_points.total_points', { ascending: false });
...
const fetchMessages = async () => { const { data, error } = await supabase .from('league_messages') .select('message, created_at, user_id, users(username)') .eq('league_id', leagueId) .order('created_at', { ascending: true });
...
return ( <div className="p-4 max-w-3xl mx-auto"> <h1 className="text-2xl font-bold mb-4">League: {leagueName}</h1>
